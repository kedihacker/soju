	...
	for _, user := range users {
		...
		for _, network := range networks {
			...
			if os.IsNotExist(err) {
				...
				log.Fatalf("unable to open network folder: %s", rootPath)
			}
			...
			root.Close()
			if err != nil {
				log.Fatalf("unable to read network folder: %s", rootPath)
			}
			...
			for _, target := range targets {
				...
				if err != nil {
					log.Fatalf("unable to open target folder: %s", targetPath)
				}

				entryNames, err := targetDir.Readdirnames(0)
				targetDir.Close()
				if err != nil {
					log.Fatalf("unable to read target folder: %s", targetPath)
				}
